...
ds<- dsL %>% dplyr::filter(year %in% c(1997)) %>% dplyr::select(id,year,byearF,bmonth,agemon,ageyear) %>%
  group_by(bmonth,byearF) %>%
  summarize(count=n()) %>% arrange(byearF,bmonth) %>%
  mutate (born=paste0(substr(byearF,3,4),"-",bmonth)) %>%
  arrange(byearF, bmonth)
head(ds, 13)
