...
```{r attend_2000, echo=echoChunks, message=T, out.width = "700px"}
...
ds<- dsL %>%
        ...
        select(id,year,yearF,attend, attendF) %>%      
        group_by(year,attendF) %>%
        summarize(count = sum(attend))

head(ds,10)
p<- ggplot(ds, aes(x=year, fill=attendF))
