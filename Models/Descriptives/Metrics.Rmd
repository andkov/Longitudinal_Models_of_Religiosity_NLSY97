...
```
Having quick access to factor labels will be especially useful during graph production. For the grammer rules of operations with relevant data see [Data Manipulation Guide][./Vignettes/dplyr/Data_Manipulation_Guide.md].
...
```{r attend_2000, echo=echoChunks, message=T, fig.height = 3.3, fig.width=7, out.width = "400px"}
...
```{r attend_2000_2011,echo=echoChunks, message=F, out.width = "700px"}
# attcol8 is defined in DeclareGlobal chunk of  the .R file
p<- ggplot(dsL, aes(x=yearF, fill=attendF))
...
```{r attend_2000_2011_na,echo=F, message=F }
p<-ggplot(subset(dsL, !is.na(attendF)), aes(x=yearF, fill=attendF))
p<-p+ geom_bar(position="fill")
p<-p+ scale_fill_manual(values = attcol8,
                    name="Response category" )
p<-p+ scale_y_continuous("Prevalence: proportion of total",
...
[5]:./Vingettes/dplyr/Data_Manipualtion_Guide.md