...
Plotting age, caclulated as  <code> age = ageyear </code>. Notice that when compared to the rounded <code>agemon</code>, variable <code>ageyear</code> provides less accurate measure of age than <code>year - byear + 1</code>
...
Plotting age, caclulated as  <code> ageD = agemon/12 </code>. This is the most precise age variable and maybe used when models require a finer age resolutions.
```{r}
ds<- dsL %>% dplyr::filter(id %in% c(4,25,35,47),year %in% c(2000:2011)) %>% 
...
[databoxList]:https://github.com/andkov/Longitudinal_Models_of_Religiosity_NLSY97/blob/master/Models/Descriptives/Databox.md
