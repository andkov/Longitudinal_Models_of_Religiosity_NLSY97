...
p<- p + geom_smooth(aes(x=timec, y=yFE,group=cohort,colour=cohort),fill=NA, linetype=1,size=1.5,alpha=1)
# scales
p<- p + scale_x_continuous(breaks=seq(0,10, 1),limits=c(0,10)) + scale_y_continuous(breaks=seq(0, 8, 1),limits=c(.5,8.5)) 
p<- p +theme(text = element_text(size =25), panel.background=element_rect(fill=bgColour,colour=NA))
# p<- p +xlab("timec.Years past since 2000")+ylab("Worship Attendance")+ggtitle(title)
p<- p +theme(legend.position=c(.95,.90),legend.direction="vertical")
p<- p +theme(legend.background = element_rect(fill=NA))
p<- p +theme(legend.text = element_text(size = 15),legend.title.align =(-3.3))
p<- p +theme(panel.grid = element_line(linetype = 1,size=rel(3)))
p
...
p<- p + geom_line(aes(y=gamma00),fill=NA, color="blue",size=2)
# p<- p + geom_line(aes(y=gamma01),fill=NA, color="red", size=2)
p<- p + geom_line(aes(y=gamma02),fill=NA, color="green, size")
p<- p + geom_line(aes(y=gamma03),fill=NA, color="blue")
p<- p + geom_line(aes(y=gamma10),fill=NA, color="blue", size=1, linetype="dashed")
p<- p + geom_line(aes(y=gamma11),fill=NA, color="red", size=1, linetype="dashed")
p<- p + geom_line(aes(y=gamma12),fill=NA, color="blue")
p<- p + geom_line(aes(y=gamma13),fill=NA, color="blue")
